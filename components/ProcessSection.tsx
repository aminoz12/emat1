        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="bg-gray-50 rounded-3xl p-8 lg:p-12">
            ...
            <div className="flex flex-wrap sm:grid sm:grid-cols-5 gap-2 mb-6 border-b border-gray-200 pb-4 overflow-x-auto">
              {['Commandes', 'Livraison', 'Retours', 'Carte grise', 'Matriculation'].map((tab) => (
                <button
                  key={tab}
                  onClick={() => {
                    setActiveTab(tab)
            ...
            <div className="space-y-4">
              {searchQuery && filteredFAQs.length === 0 ? (
                ...
                (showAllQuestions ? filteredFAQs : filteredFAQs.slice(0, 5)).map((faq, index) => {
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              {!showAllQuestions && filteredFAQs.length > 5 && (
                <button 
                  onClick={() => setShowAllQuestions(true)}
                  className="bg-gray-900 text-white px-6 py-3 rounded-2xl font-semibold text-sm hover:bg-gray-800 transition-colors"
                >
                 VOIR PLUS
                </button>
              )}
              <button className="flex items-center justify-center gap-2 text-green-600 hover:text-green-700 transition-colors font-semibold text-sm">
                ...
                Ou Parler à un agent par WhatsApp
              </button>
            </div>
          </div>
        </div>
