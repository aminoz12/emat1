-- Script pour créer une table admins séparée
-- Cette table référence les profils et permet une meilleure gestion des administrateurs
...
-- Migrer les admins existants depuis profiles vers la table admins
INSERT INTO public.admins (id, email, role, is_main_admin, can_edit_admins, can_delete_admins, is_active)
SELECT 
  id,
  email,
  role,
  CASE WHEN email = 'mhammed@ematricule.fr' THEN true ELSE false END,
  CASE WHEN role = 'SUPER_ADMIN' THEN true ELSE false END,
  CASE WHEN role = 'SUPER_ADMIN' AND email != 'mhammed@ematricule.fr' THEN true ELSE false END,
  true
